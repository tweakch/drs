...
*governance* halves of the REASONS Canvas ([`norms`](../shared/norms.md),
...
None directly. The prototype (`../../wohlen-race-analysis.html`) is *reference material*
...
- **DRS-0001 (here):** everything framework-agnostic + the *shape* of the pipeline.
...
1. **Script placeholders that exit 0** — `package.json` defines `lint`/`typecheck`/
   ...
   *Recommended* — CI workflow is final from day one; only script bodies change.
...
Prettier are *fully* active immediately (they need no app).
...
to `main` → production. So **no GitHub deploy workflow** — GH Actions owns *quality
gates*, Vercel owns *deploys*. `vercel.json` configures framework/region/build; the repo
...
- **ADRs** in `docs/adr/` (MADR-style template). Seeds: 0001 SPDD adoption, 0002 stack,
  ...
  the long-term memory of *why*, complementing the SPDD canvases' *what/how*.
